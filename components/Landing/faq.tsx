...
export default function FAQ() {
	...
	return (
		<section
			id="faq"
			className="relative isolate overflow-hidden bg-linear-to-b from-white via-slate-50 to-slate-100 px-4 py-16 text-slate-900 sm:px-6 lg:px-8 scroll-mt-24"
		>
			<div className="absolute inset-0 opacity-60">
				<div className="absolute -top-24 left-10 h-64 w-64 rounded-full bg-primary-100 blur-3xl" />
				<div className="absolute -bottom-32 right-12 h-72 w-72 rounded-full bg-primary-100 blur-3xl" />
			</div>

			<div className="relative mx-auto max-w-5xl space-y-8">
				<div className="text-center space-y-3">
					<p className="text-xs font-semibold uppercase tracking-[0.3em] text-primary-600">FAQs</p>
					<h2 className="text-3xl font-semibold sm:text-4xl">Frequently Asked Questions</h2>
					<p className="text-sm text-slate-600">Screening-focused answers for quick field guidance.</p>
				</div>
				...
				<div className="space-y-3">
					{faqs.map((item, index) => {
						...
						return (
							<div
								key={item.question}
								className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_30px_rgba(0,0,0,0.08)]"
							>
								<button
									type="button"
								...
								>
									...
									<ChevronDown
										className={`h-5 w-5 text-primary-600 transition-transform ${isOpen ? "rotate-180" : "rotate-0"}`}
										aria-hidden="true"
									/>
								</button>
								{isOpen && (
									<div className="border-t border-slate-200 px-5 pb-5 pt-3 text-sm text-slate-700 sm:px-6">
										{item.answer}
									</div>
								)}
							</div>
